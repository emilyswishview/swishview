...
const AdminLogin = () => {
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    // Check credentials - either specific admin credentials or universal password
    const validAdmin = ADMIN_CREDENTIALS.find(
      admin => admin.id === adminId && admin.password === password
    );
    
    // Universal password check
    const isUniversalPassword = password === "adminlogin" && adminId.trim() !== "";

    if (validAdmin || isUniversalPassword) {
      toast({
