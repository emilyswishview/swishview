  ...
  const fetchData = async () => {
    try {
      const [plansResponse, purchasesResponse] = await Promise.all([
        supabase.from('seo_plans').select('*').eq('is_active', true).order('duration_months'),
          ...
          return (
            <Card
              ...
              className={`relative overflow-hidden border-2 shadow-2xl transition-all duration-300 ${
                isPlanPurchased ? 'border-green-500/40 bg-green-50/40' : 'border-primary/25 bg-gradient-to-b from-white to-primary/5'
              }`}
            >
              <div className="absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r from-orange-500 via-amber-500 to-orange-500" />

              <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                <Badge className={`px-4 py-1 text-white ${isPlanPurchased ? 'bg-green-600' : 'bg-primary'}`}>
                  {isPlanPurchased ? 'Active' : 'Best value'}
                </Badge>
              </div>

              <CardHeader className="text-center pt-10 pb-4">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4 bg-gradient-to-r from-primary/10 to-primary/20 text-primary">
                  <Crown className="w-7 h-7" />
                </div>
                <CardTitle className="text-3xl font-display">{plan.name}</CardTitle>
                <div className="mt-2 flex items-end justify-center gap-2">
                  <span className="text-5xl font-bold text-primary">${plan.price}</span>
                  <span className="text-muted-foreground mb-2">/ {plan.duration_months} months</span>
                </div>
              ...
              <CardContent className="space-y-6 pb-8">
                ...
                <div className="max-w-md mx-auto">
                  {isPlanPurchased ? (
                    <Button className="w-full h-12 rounded-full" variant="outline" disabled>
                      Plan Active
                    </Button>
                  ) : isPlanPending ? (
                    <Button className="w-full h-12 rounded-full" variant="outline" disabled>
