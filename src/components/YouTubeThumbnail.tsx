  ...
  useEffect(() => {
    ...
    if (videoId) {
      ...
      const fetchThumbnail = async () => {
        try {
          const { getVideoInfo } = await import('@/utils/youtube');
          const videoInfo = await getVideoInfo(videoId);
          
          if (videoInfo?.thumbnail) {
            console.log("Using API thumbnail:", videoInfo.thumbnail);
            setThumbnailUrl(videoInfo.thumbnail);
            setVideoTitle(videoInfo.title || 'YouTube Video');
            setImageError(false);
          } else {
            // Fallback to direct YouTube thumbnail URL
            const fallbackThumbnail = `https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`;
            console.log("Using fallback thumbnail:", fallbackThumbnail);
          ...
          // Fallback to direct YouTube thumbnail URL
          const fallbackThumbnail = `https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`;
          console.log("Using fallback thumbnail after error:", fallbackThumbnail);
          setThumbnailUrl(fallbackThumbnail);
          setVideoTitle('YouTube Video');
          setImageError(false);
        }
