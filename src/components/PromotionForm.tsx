...
import { Target, DollarSign, Calendar, Users, Play, ExternalLink, Eye, TrendingUp, CheckCircle, FileSearch } from "lucide-react";
...
const PromotionForm: React.FC<CampaignFormProps> = ({ campaign, onSuccess, onCancel, targetUserId }) => {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [showAuditOffer, setShowAuditOffer] = useState(false);
  ...
  const calculateMinimumBudget = (views: number) => {
    return Math.max(50, (views * 0.02)); // $0.02 per view, minimum $50
  };

  const handleAuditRedirect = () => {
    navigate('/product#channel-audit');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const targetViews = parseInt(formData.target_views);
    const budget = parseFloat(formData.budget);
    const minimumBudget = calculateMinimumBudget(targetViews);

    if (budget < 50) {
      toast({
        title: "Budget Too Low",
        description: "Minimum budget is $50.00",
        variant: "destructive",
      });
      return;
    }
    ...
    if (budget < minimumBudget) {
      toast({
        ...
        description: `For ${targetViews.toLocaleString()} views, minimum budget is $${minimumBudget.toFixed(2)} ($0.02 per view)`,
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (campaign) {
        ...
        // Send notification email for campaign creation attempt
                    ...
                    <div className="space-y-3">
                      <Label htmlFor="budget" className="text-sm font-semibold font-display text-gray-900 flex items-center gap-2">
                       ...
                       <div className="mt-2 space-y-1">
                         <p className="text-xs text-gray-500 font-display">Minimum budget: $50.00</p>
                         <p className="text-xs text-gray-500 font-display">Rate: $0.02 per view (5,000 views = $100)</p>
                         {formData.target_views && (
                           <p className="text-xs text-orange-600 font-display font-medium">
                             Recommended budget: ${calculateMinimumBudget(parseInt(formData.target_views) || 0).toFixed(2)}
                           </p>
                         )}
                       </div>
                    </div>
          ...
          <div className="space-y-6">
            ...
            <Card className="border-2 border-primary/20 shadow-lg bg-gradient-to-br from-primary/5 to-primary/10">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <CardTitle className="text-lg font-display font-bold text-foreground">
                      Get a Channel Audit - $29
                    </CardTitle>
                    <CardDescription className="mt-1">
                      Find out exactly what's holding your channel back before starting your campaign
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="p-6 pt-0">
                ...
                >
                  Learn More About Channel Audit
                </Button>
              </CardContent>
            </Card>
          </div>
