...
function EmailCell({
  ...
  if (masked && !revealed) {
    return (
      <div className="flex items-center gap-1.5">
        <span className="text-muted-foreground">{maskEmail(value)}</span>
        <button className="opacity-60 hover:opacity-100" title="Reveal email" onClick={onReveal}>
          <Eye className="h-3.5 w-3.5" />
        </button>
      </div>
    );
  ...
  return (
    <div className="flex items-center gap-1.5">
      <a href={`mailto:${value}`} className="hover:underline truncate max-w-[180px]" title={value}>{value}</a>
      <button
        className="opacity-50 hover:opacity-100"
        title="Copy email"
      ...
      {masked && (
        <button className="opacity-60 hover:opacity-100" title="Hide email" onClick={onReveal}>
          <EyeOff className="h-3.5 w-3.5" />
        </button>
      )}
    </div>
    ...
    try {
      ...
      const lines = [
        ...
        ...allRows.map(r =>
          headers.map((_, i) => {
            const vals = [
              r.channel_name, r.channel_link, r.phone, r.email ?? "", r.subscribers ?? "", r.total_views ?? "", r.country ?? "", r.language ?? "", r.keyword ?? "", r.last_video_title ?? "", r.last_video_date ?? "", r.description ?? "", r.source, r.call_status, r.assigned_to ?? "", r.call_notes ?? "", r.last_called_at ?? "",
            ];
                ...
                <tr key={r.id} className="border-t border-border hover:bg-accent/30">
                  ...
                  <td className="p-2">
                    {r.last_video_title ? (
                      <div className="min-w-0">
                        {r.last_video_url ? (
                          <a href={r.last_video_url} target="_blank" rel="noreferrer" className="hover:underline block truncate max-w-[200px]" title={r.last_video_title}>
                            {r.last_video_title}
                          </a>
                        ) : (
                          <div className="truncate max-w-[200px]" title={r.last_video_title}>{r.last_video_title}</div>
                        )}
                  ...
                  <td className="p-2 text-muted-foreground">{r.language || "—"}</td>
