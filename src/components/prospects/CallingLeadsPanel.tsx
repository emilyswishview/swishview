...
type TextMode = "contains" | "not_contains" | "equals" | "starts" | "ends";

type ColFilter = {
  search?: string;
  mode?: TextMode;
  /** "filled" = hide empty/unfetched rows, "empty" = only rows missing this value */
...
const TEXT_MODES: { key: TextMode; label: string }[] = [
...
const ColumnHeader = ({
  col, filter, setFilter, distinctValues, onHide,
}: {
  col: CallColumn;
  ...
  const active = !!(filter && (filter.search || filter.values?.length || filter.min || filter.max || filter.sort || filter.presence));
  ...
  const presenceLabel =
    col.type === "phone" ? ["Only rows with a number", "Only rows missing a number"]
    : col.type === "email" ? ["Only rows with an email", "Only rows missing an email"]
    : ["Only filled", "Only empty (not fetched)"];

  const pickList = (isSelect ? (col.options || distinctValues) : distinctValues)
    .filter(v => !valueSearch || String(v).toLowerCase().includes(valueSearch.toLowerCase()))
    .slice(0, 300);

  const apply = (next: ColFilter) => {
    const empty = !next.search && !next.values?.length && !next.min && !next.max && !next.sort && !next.presence;
  ...
  return (
    <div className="inline-flex items-center gap-1">
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-72 p-3 space-y-2 max-h-[70vh] overflow-auto" align="start" onClick={e => e.stopPropagation()}>
          ...
          <div className="space-y-1 border-t pt-2">
            <div className="text-[10px] uppercase tracking-wide text-muted-foreground">Show rows</div>
            <div className="flex gap-1">
              <Button size="sm" variant={!local.presence ? "default" : "outline"} className="h-6 flex-1 text-[10px]"
                onClick={() => setLocal({ ...local, presence: undefined })}>All</Button>
              <Button size="sm" variant={local.presence === "filled" ? "default" : "outline"} className="h-6 flex-1 text-[10px]"
                onClick={() => setLocal({ ...local, presence: "filled" })}>{presenceLabel[0]}</Button>
              <Button size="sm" variant={local.presence === "empty" ? "default" : "outline"} className="h-6 flex-1 text-[10px]"
                onClick={() => setLocal({ ...local, presence: "empty" })}>{presenceLabel[1]}</Button>
            </div>
          </div>
          ...
          {(isNum || isDate) ? (
            ...
            <div className="space-y-1">
              <select
                className="h-7 w-full rounded-md border border-border bg-background px-1 text-xs"
                value={local.mode || "contains"}
                onChange={e => setLocal({ ...local, mode: e.target.value as TextMode })}
              >
                {TEXT_MODES.map(m => <option key={m.key} value={m.key}>{m.label}</option>)}
              </select>
              <Input
                className="h-7 text-xs" placeholder="Value…"
                value={local.search || ""} onChange={e => setLocal({ ...local, search: e.target.value })}
                onKeyDown={e => { if (e.key === "Enter") { apply(local); setOpen(false); } }}
              />
            </div>
          )}

          {(isSelect || pickList.length > 0) && (
            <div className="space-y-1 border-t pt-2">
              <Input
                className="h-7 text-xs" placeholder="Search values…"
                value={valueSearch} onChange={e => setValueSearch(e.target.value)}
              />
              <div className="flex gap-1">
                <Button size="sm" variant="outline" className="h-5 flex-1 text-[10px]"
                  onClick={() => setLocal({ ...local, values: pickList.slice() })}>Select all</Button>
                <Button size="sm" variant="outline" className="h-5 flex-1 text-[10px]"
                  onClick={() => setLocal({ ...local, values: [] })}>None</Button>
              </div>
...
export default function CallingLeadsPanel({
  ...
  const [hiddenCols, setHiddenCols] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem("calling.hiddenCols") || "[]"); } catch { return []; }
  });
  const [autoFetch, setAutoFetch] = useState<boolean>(
    () => localStorage.getItem("calling.autoFetch") === "1",
  );
  const [autoStats, setAutoStats] = useState<{ done: number; running: boolean }>({ done: 0, running: false });

  useEffect(() => { localStorage.setItem("calling.hiddenCols", JSON.stringify(hiddenCols)); }, [hiddenCols]);
  useEffect(() => { localStorage.setItem("calling.autoFetch", autoFetch ? "1" : "0"); }, [autoFetch]);

  const allowedColumns = useMemo(
    () => COLUMNS.filter(c =>
      (!c.adminOnly || isAdmin) &&
      !(maskContacts && ["phone", "email"].includes(c.key as string))
    ),
    [isAdmin, maskContacts],
  );

  const visibleColumns = useMemo(
    () => allowedColumns.filter(c => !hiddenCols.includes(c.key)),
    [allowedColumns, hiddenCols],
  );
  ...
  // ── Background worker: keeps completing incomplete leads while the tab is open.
  const enrichRef = React.useRef(enrichLead);
  enrichRef.current = enrichLead;

  useEffect(() => {
    if (!autoFetch) { setAutoStats(s => ({ ...s, running: false })); return; }
    let stop = false;
    setAutoStats(s => ({ ...s, running: true }));
      ...
      while (!stop) {
        try {
          const { data } = await supabase
            ...
            .or("channel_name.is.null,subscribers.is.null,thumbnail.is.null,country.is.null,channel_joined.is.null,last_video_title.is.null,total_views.is.null")
            .limit(20);
          const batch = ((data || []) as CallingLead[]).filter(r => r.channel_link);
          if (!batch.length) { await sleep(60_000); continue; }
          for (const row of batch) {
            if (stop) break;
            const ok = await enrichRef.current(row, true);
            if (ok) setAutoStats(s => ({ ...s, done: s.done + 1 }));
            await sleep(1200);
          }
        } catch {
          await sleep(15_000);
    ...
    Object.entries(filters).forEach(([key, f]) => {
      ...
      if (f.presence) {
        const meaningful = (v: string) => {
          const t = v.trim();
          if (!t || t === "0" || t.toUpperCase() === "NONE" || t === "-" || t === "—") return false;
          const col = COLUMNS.find(c => c.key === key);
          if (col?.type === "phone") return t.replace(/\D/g, "").length >= 7;
          if (col?.type === "email") return t.includes("@");
          return true;
        };
        out = out.filter(r => (f.presence === "filled" ? meaningful(cellText(r, key)) : !meaningful(cellText(r, key))));
      }
      if (f.search) {
        const needle = f.search.toLowerCase();
        const mode = f.mode || "contains";
        out = out.filter(r => {
          const v = cellText(r, key).toLowerCase();
          switch (mode) {
            case "not_contains": return !v.includes(needle);
            case "equals": return v === needle;
            case "starts": return v.startsWith(needle);
            case "ends": return v.endsWith(needle);
    ...
    <div className="w-full h-[calc(100vh-180px)] flex flex-col overflow-hidden gap-2">
      <div className="flex shrink-0 items-center gap-2 overflow-x-auto pb-1 thin-scrollbar">
        ...
        {isAdmin && (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="max-h-80 w-64 overflow-auto">
              <DropdownMenuItem onSelect={e => { e.preventDefault(); setHiddenCols([]); }}>
                <Eye className="mr-2 h-3.5 w-3.5" /> Show all columns
              </DropdownMenuItem>
              ...
              {allowedColumns.map(c => (
                ...
                >
                  <Checkbox checked={!hiddenCols.includes(c.key)} className="pointer-events-none" />
                  {c.label}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
        ...
        <Button
          ...
          title="Continuously fetch missing YouTube details in the background"
        >
          {autoFetch ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />}
          {autoFetch ? `Auto-fetch on${autoStats.done ? ` · ${autoStats.done}` : ""}` : "Auto-fetch off"}
        </Button>
        {activeColFilters > 0 && (
              ...
              {visibleColumns.map(col => (
                ...
                >
                  <ColumnHeader
                    ...
                    onHide={isAdmin ? () => toggleColumn(col.key) : undefined}
                  />
                </th>
              ))}
