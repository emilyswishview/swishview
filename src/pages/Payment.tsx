...
const Payment = () => {
  ...
  const handleStripePayment = async () => {
    ...
    try {
      ...
      // Derive billing cycle in months from the campaign package length.
      // `campaign_duration` is stored in days; fall back to 1 month if missing.
      const days = Number(campaign?.campaign_duration) || 30;
      const durationMonths = Math.min(Math.max(Math.round(days / 30), 1), 12);
