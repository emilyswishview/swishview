...
import { fetchAndStoreInitialViews } from "@/utils/youtube";
...
const CreateCampaign = () => {
  ...
  const handleVideoUrlChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (validateYouTubeUrl(url)) {
      ...
      // Always show preview immediately with thumbnail URL
      setVideoPreview({
        id: videoId,
      ...
      // Test thumbnail in background but don't block the UI
      if (thumbnailUrl) {
        try {
          const img = new Image();
          img.onload = () => {
          ...
          img.onerror = () => {
            // Thumbnail failed to load, show placeholder but keep preview
            setVideoPreview({
              id: videoId,
              thumbnail: '/placeholder.svg', // Use default placeholder
              url: url
            });
            setThumbnailFailed(false); // Don't force upload, just use placeholder
          };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Fetch initial view count from YouTube using the secure edge function
      console.log('Fetching initial view count for new campaign...');
      const initialViews = await fetchAndStoreInitialViews(formData.youtube_video_url);
      
      if (initialViews === null) {
        toast({
          title: "Warning",
          description: "Could not fetch initial view count from YouTube. Please check the video URL and try again.",
          variant: "destructive",
        });
        setLoading(false);
        return;
      ...
      console.log('Creating campaign with initial views:', initialViews);
      ...
      // Send notification email for campaign creation
      ...
      toast({
        ...
        description: `Campaign created with initial view count: ${initialViews.toLocaleString()}. Complete payment to activate it.`,
      });
