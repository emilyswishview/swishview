...
WhatsApp - +1 (705) 614 0340`;
  ...
  const syncAll = async () => {
    ...
    // Count rows across ALL pages that still need today's sync. Predicate must
    // exactly match the edge function's "needs sync" predicate so the progress
    // bar reaches 0 when the worker is done.
    ...
    const countPending = async (): Promise<number> => {
      let q: any = supabase
        ...
        .eq("is_banned", false)
        .or(`last_fetched_at.is.null,last_fetched_at.lt.${startISO}`);
      if (ownerSenders) q = q.in("assigned_sender", ownerSenders);
      const { count } = await q;
      return count || 0;
    };
    ...
    // Auto-dedupe across the entire database (admin only) so newly imported
    // sheets get cleaned up automatically.
    let dedupedCount = 0;
    if (isAdmin) {
      try {
        const { data: dd } = await supabase.rpc("prospects_dedupe_all" as any);
        const row = Array.isArray(dd) ? dd[0] : dd;
        dedupedCount = Number((row as any)?.removed) || 0;
      } catch (e) { console.error("auto-dedupe failed", e); }
    }

    toast({
      title: cancelSyncRef.current ? "Sync stopped" : `Synced ${initialPending} row${initialPending === 1 ? "" : "s"}`,
      description: dedupedCount > 0 ? `Removed ${dedupedCount} duplicate row${dedupedCount === 1 ? "" : "s"}` : undefined,
    });
  };
    ...
    <div className="h-screen flex flex-col bg-background text-foreground overflow-hidden">
      ...
      <div className="border-b border-border bg-card shrink-0 z-30">
        ...
        <div className="px-4 py-3 flex items-center gap-3 flex-wrap">
          ...
          {role && (
            <Button size="sm" variant="ghost" onClick={signOut} className="h-9 px-2 gap-1.5" title="Sign out">
