...
const pathToTab = (pathname: string): ProspectsTab | null => {
  if (pathname.startsWith("/prospects/unqualified")) return "unqualified";
...
import {
  ...
  Eye, EyeOff, Copy, CopyMinus, ClipboardCopy, ChevronLeft, ChevronRight, ChevronDown, Mail, Send, Reply, Paperclip, Sparkles, Pencil, Wand2, Upload, PhoneCall,
} from "lucide-react";


import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer } from "recharts";
import { useProspectsSession, PROSPECTS_ALLOWED_SENDERS } from "@/hooks/useProspectsSession";
const EmployeePermissionsDialog = React.lazy(() => import("@/components/prospects/EmployeePermissionsDialog"));
...
// Sales reps that receive auto-distributed leads (50/50 split).
// Assignment is deterministic from the prospect id so it never changes
// across reloads and stays balanced across the full dataset.
const CLOSERS = ["Adarsh", "Shivam"] as const;
const getCloser = (id: string): string => {
  let h = 0;
  for (let i = 0; i < id.length; i++) h = (h * 31 + id.charCodeAt(i)) | 0;
  return CLOSERS[Math.abs(h) % CLOSERS.length];
};
...
const COLUMNS: Column[] = [
  ...
  { key: "closer" as any, label: "Assigned Closer", width: 130, type: "text", auto: true },
...
const DEFAULT_SENDERS = [
  ...
  { value: "sales1@swishview.com", display: "Sales 1" },
  { value: "sales2@swishview.com", display: "Sales 2" },
  { value: "sales3@swishview.com", display: "Sales 3" },
  ...
  { value: "noelle@swishview.email", display: "Noelle (.email)" },
...
const Prospects = () => {
  ...
  const [sendQueueOpen, setSendQueueOpen] = useState(() => typeof window !== "undefined" && window.location.pathname.startsWith("/prospects/queue"));
  ...
  // Tab lives in the URL: /prospects/leads · /unqualified · /banned ·
  // /bounces · /calling. Bare /prospects (and /prospects/queue) fall back to
  // the last tab used on this device.
  const location = useLocation();
  const routeTab = pathToTab(location.pathname);
  const [storedTab, setStoredTab] = useState<ProspectsTab>(() => {
    const v = localStorage.getItem("prospects.sourceFilter") as ProspectsTab | null;
    return v && v in TAB_TO_PATH ? v : "prospects";
  });
  const sourceFilter: ProspectsTab = routeTab ?? storedTab;
  const setSourceFilter = React.useCallback((tab: ProspectsTab) => {
    setStoredTab(tab);
    const target = TAB_TO_PATH[tab];
    if (!window.location.pathname.startsWith(target)) navigate(target);
  }, [navigate]);
  useEffect(() => { if (routeTab) setStoredTab(routeTab); }, [routeTab]);
  useEffect(() => { localStorage.setItem("prospects.sourceFilter", sourceFilter); }, [sourceFilter]);
  // Keep the Send Queue panel in sync with /prospects/queue.
  useEffect(() => {
    setSendQueueOpen(location.pathname.startsWith("/prospects/queue"));
  }, [location.pathname]);
  const { isAdmin, isViewer, maskContacts, role, email: authedEmail, signOut, perms } = useProspectsSession();
  const [permsDialogOpen, setPermsDialogOpen] = useState(false);
  // Viewers (Marcus) get the full admin UI but zero action permissions.
  const canEdit = !isViewer && (isAdmin || perms.can_edit);
  const canCreate = !isViewer && (isAdmin || perms.can_create);
  const canDelete = !isViewer && (isAdmin || perms.can_delete);
  const canSend = !isViewer && (isAdmin || perms.can_send);
  const canSync = !isViewer && (isAdmin || perms.can_sync);
  const canBan = !isViewer && (isAdmin || perms.can_ban);
  const canExport = !isViewer && (isAdmin || perms.can_export);
  ...
  useEffect(() => {
    if (!isAdmin && (sourceFilter === "banned" || sourceFilter === "discovered" || sourceFilter === "bounced")) {
  ...
  // Admin-only: reconcile bounced leads on mount so the Prospects count
  // never includes hard-bounced rows that haven't been banned yet. Runs
  // once per session; the sync function bans matching prospects.
  useEffect(() => {
    if (!isAdmin) return;
    const KEY = "prospects.bounceReconciledAt";
    const last = Number(localStorage.getItem(KEY) || "0");
    if (Date.now() - last < 6 * 3600_000) return; // at most once every 6h
    localStorage.setItem(KEY, String(Date.now()));
    supabase.functions.invoke("sync-email-bounces", {
      body: { days: 90, max: 5000, removeFromProspects: true },
    }).catch(() => {});
  }, [isAdmin]);

  // Admin-only: rebalance assigned_sender across reps. Targets rows with
  // no sender or with the removed growth@swishview.com address.
  const [rebalancing, setRebalancing] = useState(false);
  const rebalanceAssignedSenders = async () => {
    if (!isAdmin || rebalancing) return;
    if (!confirm("Reassign every unassigned lead (and growth@swishview.com leads) across the remaining reps?")) return;
    setRebalancing(true);
    try {
      const REPS = SALES.filter(s => s && s !== "growth@swishview.com");
      const all: { id: string }[] = [];
      const PAGE = 1000;
      let from = 0;
      while (true) {
        const { data, error } = await supabase
          ...
          .select("id")
          .eq("is_banned", false)
          .or("assigned_sender.is.null,assigned_sender.eq.,assigned_sender.eq.growth@swishview.com")
      ...
      if (!all.length) { toast({ title: "Nothing to rebalance", description: "Every lead already has a sales rep." }); return; }
      toast({ title: `Rebalancing ${all.length.toLocaleString()} leads…` });
      // Round-robin across REPS, batched in chunks of 100 per rep.
      const buckets: Record<string, string[]> = Object.fromEntries(REPS.map(r => [r, []]));
      all.forEach((row, i) => buckets[REPS[i % REPS.length]].push(row.id));
      let done = 0;
      for (const rep of REPS) {
        const ids = buckets[rep];
        for (let i = 0; i < ids.length; i += 100) {
          const slice = ids.slice(i, i + 100);
          const { error } = await supabase
            .from("prospects")
            .update({ assigned_sender: rep })
            .in("id", slice);
          if (error) throw error;
          done += slice.length;
        }
      }
      toast({ title: "Rebalance complete", description: `${done.toLocaleString()} leads reassigned across ${REPS.length} reps.` });
    } catch (e: any) {
      toast({ title: "Rebalance failed", description: e?.message || String(e), variant: "destructive" });
    } finally {
...
WhatsApp - +1 (705) 614 0340`;
  ...
  // Rows per page — user-selectable and remembered per device.
  const [PAGE_SIZE, setPageSize] = useState<number>(() => {
    const v = parseInt(localStorage.getItem("prospects.pageSize") || "200", 10);
    return [50, 100, 200, 500, 1000].includes(v) ? v : 200;
  });
  useEffect(() => { localStorage.setItem("prospects.pageSize", String(PAGE_SIZE)); }, [PAGE_SIZE]);
  ...
  const visibleColumns = useMemo(() => COLUMNS.filter(c =>
    !hiddenCols.has(c.key as string) &&
    !(maskContacts && ["email", "altEmail", "phone", "emailHistory"].includes(c.key as string))
  ), [hiddenCols, maskContacts]);
  ...
  useEffect(() => {
    let cancelled = false;
    // `count: "exact"` runs a serialized COUNT(*) on the matching set which on
    // large tabs (banned, all-prospects) routinely exceeds the statement
    // timeout and bricks the page load. Estimated count is near-instant and
    // good enough for pagination UI.
    ...
    const LIST_COLS = "id, assigned_sender, is_banned, auto_discovered, data";
    const buildQuery = (from: number, to: number, withCount: boolean) => {
      let q: any = supabase
        .from("prospects")
        .select(LIST_COLS, withCount ? { count: "exact" } : (undefined as any));
      if (serverSort) {
        q = q.order(serverSort.col, { ascending: serverSort.asc, nullsFirst: false });
      } else {
        // Default: newest video first so freshly active channels surface at the top.
        q = q.order("last_video_date", { ascending: false, nullsFirst: false })
    ...
    return () => { cancelled = true; };
  }, [currentPage, PAGE_SIZE, debouncedSearch, sourceFilter, onlyWithConv, onlyWithPhone, serverSort, hasColumnFilters, isAdmin, authedEmail]);
  ...
  const upsertRow = async (row: Prospect, isNew = false) => {
    const payload: any = { data: row as any };
    ...
    // IMPORTANT: use UPDATE for existing rows. `upsert` runs an INSERT ..
    // ON CONFLICT which is checked against the INSERT RLS policy (admin only),
    // so sales reps got "new row violates row-level security policy".
    const { error } = isNew
      ? await supabase.from("prospects").insert({ id: row.id, ...payload })
      : await supabase.from("prospects").update(payload).eq("id", row.id);
  ...
  const unbanSelected = async () => {
    ...
    const ids = Array.from(selected);
    if (!ids.length) return;
    const { error } = await supabase.from("prospects").update({ is_banned: false } as any).in("id", ids);
  ...
  const applyBulkEdit = async () => {
    ...
    // Use UPDATE (not upsert) so non-admin reps aren't blocked by the
    // admin-only INSERT policy on prospects.
    const CONC = 10;
    let failed = 0;
    for (let i = 0; i < updated.length; i += CONC) {
      const chunk = updated.slice(i, i + CONC);
      const res = await Promise.all(chunk.map(p =>
        supabase.from("prospects").update({ data: p as any }).eq("id", p.id)
      ));
      failed += res.filter(r => r.error).length;
    }
    if (failed) toast({ title: `${failed} rows failed to save`, variant: "destructive" });
    toast({ title: `Updated ${updated.length - failed} rows: ${bulkField} → "${bulkValue || "(empty)"}"` });
  ...
  const dedupe = async () => {
    ...
    if (!confirm("Run dedupe across the entire database? This removes duplicate rows that share the same email, keeping the row with the richest data.")) return;
    const t = toast({ title: "Dedupe running…" });
    try {
      // Live DB exposes `prospects_dedupe_by_email`, which returns the integer
      // number of rows removed. (The old `prospects_dedupe_all` was never
      // deployed to this project, so calling it failed with PGRST202.)
      const { data, error } = await supabase.rpc("prospects_dedupe_by_email" as any);
      if (error) throw error;
      const row = Array.isArray(data) ? data[0] : data;
      // Function may return a bare integer (removed count) or an object.
      const removed = typeof row === "number"
        ? row
        : Number((row as any)?.removed ?? (row as any)?.prospects_dedupe_by_email ?? 0);
      t.update({
        id: t.id,
        title: removed > 0 ? `Removed ${removed} duplicate row${removed === 1 ? "" : "s"}` : "No duplicates found",
        description: removed > 0 ? "Duplicate emails merged into the richest row." : undefined,
      });
  ...
  useEffect(() => {
    ...
    // Workspace contacts auto-discovery is disabled — it was scanning the
    // entire email_tracker_logs table on every load. Admin must trigger it
    // manually if a refresh is ever needed.
  ...
  const fetchYouTubeForRow = async (row: Prospect): Promise<Prospect | null> => {
    ...
    const keepPhone = existingPhone && existingPhone.toUpperCase() !== "NONE";

    return {
      ...row,
      phone: keepPhone ? existingPhone : (data.phone || "NONE"),
  ...
  // Kick off the background phone-number discovery pass. The edge function
  // walks every row missing a phone, scrapes YouTube for one, and writes
  // "NONE" when nothing is found so it moves on to the next lead.
  // While it runs we poll live counters (found / checked / total) and patch
  // fresh numbers straight into the visible rows.
  const [findingPhones, setFindingPhones] = useState(false);
  const [phoneStats, setPhoneStats] = useState<{ total: number; checked: number; found: number }>({ total: 0, checked: 0, found: 0 });
  const phonePollRef = useRef<number | null>(null);

  const phoneScopeQuery = useCallback((select: string) => {
    const myEmail = (authedEmail || "").toLowerCase().trim();
    const ownerSenders = isAdmin ? null : (PROSPECTS_ALLOWED_SENDERS[myEmail] || (myEmail ? [myEmail] : null));
    let q: any = supabase
      .from("prospects")
  ...
  const refreshPhoneStats = useCallback(async () => {
    try {
      const [totalRes, checkedRes, foundRes] = await Promise.all([
        phoneScopeQuery("id"),
        phoneScopeQuery("id").not("data->>phone", "is", null).neq("data->>phone", ""),
        phoneScopeQuery("id").not("data->>phone", "is", null).neq("data->>phone", "").neq("data->>phone", "NONE"),
      ]);
      setPhoneStats({
  ...
  // Pull the latest phone values for the rows currently on screen so numbers
  // appear live while the scan runs.
  const refreshVisiblePhones = useCallback(async () => {
    const ids = rows.slice(0, 400).map(r => r.id).filter(Boolean);
    if (!ids.length) return;
    const { data } = await supabase.from("prospects").select("id, data").in("id", ids);
    if (!data?.length) return;
    const byId = new Map<string, string>(data.map((r: any) => [r.id, String(r.data?.phone || "")]));
    setRows(prev => prev.map(r => {
      const p = byId.get(r.id);
      return p !== undefined && p !== r.phone ? { ...r, phone: p } : r;
    }));
  }, [rows]);

  useEffect(() => { refreshPhoneStats(); }, [refreshPhoneStats]);

  useEffect(() => {
    if (!findingPhones) return;
    let ticks = 0;
    const id = window.setInterval(async () => {
      ticks++;
      await refreshPhoneStats();
      if (ticks % 2 === 0) await refreshVisiblePhones();
      // Stop showing "scanning" once every lead has been checked.
      setPhoneStats(s => {
        if (s.total > 0 && s.checked >= s.total) setFindingPhones(false);
        return s;
      });
      // Safety stop after 30 minutes of polling.
      if (ticks > 360) setFindingPhones(false);
    }, 5000);
    phonePollRef.current = id;
    return () => window.clearInterval(id);
  }, [findingPhones, refreshPhoneStats, refreshVisiblePhones]);

  const findPhones = async () => {
    const myEmail = (authedEmail || "").toLowerCase().trim();
    const ownerSenders = isAdmin ? null : (PROSPECTS_ALLOWED_SENDERS[myEmail] || (myEmail ? [myEmail] : null));
    if (findingPhones) { setFindingPhones(false); return; }
    setFindingPhones(true);
    await refreshPhoneStats();
    try {
      const { error } = await supabase.functions.invoke("prospects-find-phones", {
        body: { ownerSenders },
      });
      if (error) throw error;
    } catch (e: any) {
      setFindingPhones(false);
      toast({ title: "Could not start phone scan", description: e?.message || String(e), variant: "destructive" });
    }
  };
  ...
  const exportXlsx = () => {
    const data = rows.map(r => {
      ...
      COLUMNS.forEach(c => {
        ...
        } else if ((c.key as string) === "closer") {
          o[c.label] = getCloser(r.id);
        } else {
  ...
  const filteredSorted = useMemo(() => {
    ...
    return out;
  }, [rows, search, filters, formulaFn, onlyWithConv, onlyWithPhone, convCounts, sourceFilter, prospectsFirst]);
...
${vidBlock(2)}`;
  ...
  const clientPaging = hasColumnFilters || onlyWithConv || onlyWithPhone;
  ...
  // Page navigation always jumps the table back to the first row so the user
  // never lands mid-scroll on a new page.
  const goToPage = React.useCallback((p: number) => {
    setCurrentPage(Math.max(0, Math.min(totalPages - 1, p)));
    requestAnimationFrame(() => scrollRef.current?.scrollTo({ top: 0 }));
  }, [totalPages]);

  // Keyboard paging: Option/Alt + ← / → (and + Home / End), ignored while typing.
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (!e.altKey) return;
      const t = e.target as HTMLElement | null;
      if (t && /^(INPUT|TEXTAREA|SELECT)$/.test(t.tagName)) return;
      if (e.key === "ArrowRight") { e.preventDefault(); goToPage(safePage + 1); }
      else if (e.key === "ArrowLeft") { e.preventDefault(); goToPage(safePage - 1); }
      else if (e.key === "Home") { e.preventDefault(); goToPage(0); }
      else if (e.key === "End") { e.preventDefault(); goToPage(totalPages - 1); }
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [goToPage, safePage, totalPages]);

  // Compact page-number window around the current page.
  const pageWindow = React.useMemo(() => {
    const span = 2;
    const from = Math.max(0, Math.min(safePage - span, totalPages - (span * 2 + 1)));
    const to = Math.min(totalPages - 1, Math.max(safePage + span, span * 2));
    const out: number[] = [];
  ...
  useEffect(() => { setCurrentPage(0); }, [formulaExpr, onlyWithConv, onlyWithPhone, sourceFilter]);
    ...
    <div className="h-screen flex flex-col bg-background text-foreground overflow-hidden">
      ...
      <div className="border-b border-border bg-card shrink-0 z-30">
        ...
        <div className="px-4 py-3 flex items-center gap-3 flex-wrap">
          ...
          <Button
            size="icon" variant="ghost" className="h-9 w-9"
            onClick={() => navigate("/prospects/queue")}
          ...
          <button
            ...
            title={
              findingPhones
                ? `Scanning ${phoneStats.checked.toLocaleString()}/${phoneStats.total.toLocaleString()} — ${phoneStats.found.toLocaleString()} numbers found. Click to stop watching.`
                : `Find phone numbers — scans YouTube About, links and the creator's website. ${phoneStats.found.toLocaleString()} of ${phoneStats.total.toLocaleString()} leads have a number.`
            }
            className="inline-flex items-center gap-1.5 h-9 px-2 rounded text-muted-foreground hover:text-foreground hover:bg-accent"
          >
            {findingPhones
              ? <Loader2 className="h-3.5 w-3.5 animate-spin text-primary" />
              : <PhoneCall className="h-3.5 w-3.5" />}
            <span className="text-[11px] tabular-nums">
              {phoneStats.found.toLocaleString()}/{phoneStats.total.toLocaleString()}
              {findingPhones && phoneStats.total > 0 && (
                <span className="text-muted-foreground/70"> · {Math.min(100, Math.round((phoneStats.checked / Math.max(1, phoneStats.total)) * 100))}%</span>
              )}
            </span>
          </button>
          <Button
          ...
          <div className="inline-flex items-center rounded-md border border-border bg-background p-0.5" role="tablist">
            ...
            {/* Workspace Contacts tab intentionally hidden — auto-discovery
                of Workspace counterparties was bloating the page load with
                no day-to-day value. Admin can still trigger
                `discoverFromWorkspace()` manually from the console if a
                bulk import is ever needed. */}
            {isAdmin && (
              <button
                type="button"
                role="tab"
            ...
            {isAdmin && (
              <button
                type="button"
                role="tab"
                aria-selected={sourceFilter === "bounced"}
                onClick={() => setSourceFilter("bounced")}
                className={`px-2.5 h-7 text-xs rounded ${sourceFilter === "bounced" ? "bg-orange-500/15 text-orange-700 dark:text-orange-400" : "text-muted-foreground hover:bg-accent/50"}`}
                title="Bounced emails ingested from Gmail Mailer-Daemon notifications"
              >
                Bounced
              </button>
            )}
            <button
              type="button"
              role="tab"
              aria-selected={sourceFilter === "calling"}
              onClick={() => setSourceFilter("calling")}
              className={`px-2.5 h-7 text-xs rounded ${sourceFilter === "calling" ? "bg-green-500/15 text-green-700 dark:text-green-400" : "text-muted-foreground hover:bg-accent/50"}`}
              title="Calling leads — phone numbers collected by the /phone tool"
            >
              Calling
            </button>
            {(
      ...
      {sourceFilter === "users" ? (
        ...
        <div className="flex-1 min-h-0 overflow-auto p-4">
          <React.Suspense fallback={<div className="p-6 text-xs text-muted-foreground">Loading calling leads…</div>}>
            <CallingLeadsPanel canDelete={canDelete} isAdmin={isAdmin} currentEmail={authedEmail} maskContacts={maskContacts} />
          </React.Suspense>
        </div>
      ) : sourceFilter === "bounced" ? (
        <React.Suspense fallback={<div className="p-6 text-xs text-muted-foreground">Loading bounced emails…</div>}>
          <BouncedEmailsPanel />
        </React.Suspense>
      ) : (
      <div
                ...
                >
                  <ColumnHeader
                    ...
                    extraToggle={col.type === "emails" ? {
                      ...
                      label: "Only leads with a phone number",
                      checked: onlyWithPhone,
                      onChange: setOnlyWithPhone,
                      count: phoneStats.found,
                      onSync: findPhones,
                      syncing: findingPhones,
                    } : undefined}
                  />
            ...
            {virtualItems.map((virtualRow) => {
              ...
              <tr key={row.id} className={`hover:bg-muted/30 ${row.autoDiscovered ? "bg-amber-50/60 dark:bg-amber-950/20 border-l-2 border-amber-400" : ""}`} style={{ height: 34 }} title={row.autoDiscovered ? "Auto-discovered from Google Workspace conversations" : undefined}>
                ...
                {visibleColumns.map(col => {
                  ...
                  if ((col.key as string) === "closer") {
                    const c = getCloser(row.id);
                    return (
                      <td key="closer" className="border border-border p-1 text-center" style={{ width: col.width, minWidth: col.width }}>
                        <span className={`inline-flex items-center justify-center px-2 h-6 rounded text-[11px] font-medium ${c === "Adarsh" ? "bg-blue-100 text-blue-800" : "bg-emerald-100 text-emerald-800"}`}>
                          {c}
                        </span>
                      </td>
                    );
      ...
      {/* Pagination footer — only for the prospect table tabs */}
      {sourceFilter !== "calling" && sourceFilter !== "bounced" && (
      <div className="border-t border-border bg-card px-4 py-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 justify-between text-[11px] shrink-0">
        <div className="text-muted-foreground flex items-center gap-3">
          <span>
            {total === 0 ? "0 rows" : (
              <>
                Showing <strong>{startIdx + 1}</strong>–<strong>{endIdx}</strong> of <strong>{total.toLocaleString()}</strong>
                {(hasColumnFilters || onlyWithConv || onlyWithPhone) && <> filtered (of <strong>{serverTotal.toLocaleString()}</strong> total)</>}
              </>
            )}
          </span>
          <span className="hidden md:inline text-muted-foreground/70">⌥ Option + ← / → to page</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            {pageWindow.map(p => (
              <button
                key={p}
                onClick={() => goToPage(p)}
                className={`h-6 min-w-6 px-1 inline-flex items-center justify-center rounded tabular-nums ${
                  p === safePage ? "bg-primary text-primary-foreground" : "hover:bg-accent text-muted-foreground"
                }`}
                title={`Page ${p + 1}`}
              >{p + 1}</button>
            ))}
      ...
      {sendQueueOpen && (
        <React.Suspense fallback={null}>
          <SendQueuePanel
            ...
            onOpenChange={(o) => {
              setSendQueueOpen(o);
              if (!o && window.location.pathname.startsWith("/prospects/queue")) {
                navigate(TAB_TO_PATH[sourceFilter]);
              }
            }}
