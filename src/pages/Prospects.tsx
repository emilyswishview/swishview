...
import {
  ...
  Eye, EyeOff, Copy, CopyMinus, ClipboardCopy, ChevronLeft, ChevronRight, ChevronDown, Mail, Send, Reply, Paperclip, Sparkles, Pencil, Wand2,
} from "lucide-react";
...
const ProspectEmails = ({ emails, clientName, prospect }: ProspectEmailsProps) => {
  ...
  const defaultFrom = (prospect?.assignedSender || "").toLowerCase().trim() || DEFAULT_SENDERS[0].value;
                  ...
                  {selectedThread.messages.map((m, i) => (
                    <div key={m.id} className="border border-border rounded-lg p-3 bg-card">
                      ...
                      <div className="flex flex-wrap gap-1 text-[10px]">
                        ...
                        {m.read_at && <span className="bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-200 rounded px-1.5 py-0.5">Read {fmtDate(m.read_at)}</span>}
                        {m.replied_at && <span className="bg-emerald-100 text-emerald-700 dark:bg-emerald-900 dark:text-emerald-200 rounded px-1.5 py-0.5">Replied {fmtDate(m.replied_at)}</span>}
                        {m.forwarded_at && <span className="bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-200 rounded px-1.5 py-0.5">Forwarded</span>}
                        {m.delivery_status && m.delivery_status !== "delivered" && <span className="bg-amber-100 text-amber-700 rounded px-1.5 py-0.5">{m.delivery_status}</span>}
                        {m.has_attachments && (
...
const Prospects = () => {
  ...
  const [sourceFilter, setSourceFilter] = useState<"prospects" | "discovered" | "banned">(
    () => {
...
WhatsApp - +1 (705) 614 0340`;
  ...
  useEffect(() => {
    ...
    const buildQuery = (from: number, to: number, withCount: boolean) => {
      ...
      else if (sourceFilter === "prospects") q = q.eq("auto_discovered", false);
      return q;
    };
    (async () => {
      setLoading(true);
    ...
    return () => { cancelled = true; };
  }, [currentPage, debouncedSearch, sourceFilter, onlyWithConv, serverSort, hasColumnFilters]);
  ...
  const banSelected = async () => {
    ...
    const reason = window.prompt("Ban reason (incorrect email, channel deleted, bounced, etc.):", "");
    if (reason === null) return;
    const { error } = await supabase.from("prospects").update({ is_banned: true, ban_reason: reason || null } as any).in("id", ids);
    if (error) { toast({ title: "Ban failed", description: error.message, variant: "destructive" }); return; }
    setRows(prev => prev.filter(r => !ids.includes(r.id)));
    setSelected(new Set());
    toast({ title: `Banned ${ids.length} lead${ids.length === 1 ? "" : "s"}` });
  };
  ...
  const openBulkAI = async () => {
    // If a queue already exists, ask the user whether to keep it or replace
    // it with the currently selected leads. "No" keeps the old queue intact
    // and ignores the new selection.
    if (bulkDrafts.length > 0) {
      ...
      if (isNewSelection) {
        const replace = confirm(
          `You already have ${bulkDrafts.length} email${bulkDrafts.length === 1 ? "" : "s"} in the queue.\n\n` +
          `OK = Discard the current queue and load the ${selIds.length} newly selected lead${selIds.length === 1 ? "" : "s"}.\n` +
          `Cancel = Keep the existing queue (ignore new selection).`
        );
        if (replace) {
          setBulkDrafts([]);
  ...
  const dedupe = async () => {
    const score = (r: Prospect) => Object.values(r).filter(v => v && String(v).length > 0).length;
    const norm = (s?: string) => (s || "").toLowerCase().trim();
    // Union-find by every email key present on the row
    ...
    rows.forEach(r => {
      const keys = [norm(r.email), norm(r.altEmail)].filter(Boolean);
      if (!keys.length) return;
      rowKeys[r.id] = keys;
      keys.forEach(k => { if (!(k in parent)) parent[k] = k; });
      for (let i = 1; i < keys.length; i++) union(keys[0], keys[i]);
    });
    // Group rows by the union-find root of their first email key
    const groups: Record<string, Prospect[]> = {};
    rows.forEach(r => {
      const keys = rowKeys[r.id];
      if (!keys) return;
    ...
    Object.values(groups).forEach(g => {
      ...
      const keep = g.reduce((a, b) => score(a) >= score(b) ? a : b);
      g.forEach(r => { if (r.id !== keep.id) toDelete.push(r.id); });
    });
    if (!toDelete.length) { toast({ title: "No duplicates found" }); return; }
    if (!confirm(`Remove ${toDelete.length} duplicate row(s)?`)) return;
  ...
  const refreshConversations = async (silent = false, force = false) => {
    ...
    rows.forEach(r => {
      [r.email, r.altEmail].filter(Boolean).forEach(e => {
    ...
    if (!silent) {
      ...
      toast({
        title: "Sync complete",
        description: `${withConv}/${rows.length} prospects · ${totalFetched} messages`,
      });
    }
  };
  ...
  const syncAll = async () => {
    ...
    // Count how many rows across ALL pages still need today's sync.
    ...
    const countPending = async (): Promise<number> => {
      const { count } = await supabase
        ...
        .neq("channel_link", "")
        .or(
          `last_fetched_at.is.null,last_fetched_at.lt.${startISO},channel_name.is.null,channel_name.eq.`
        );
      return count || 0;
    };
    ...
    if (initialPending === 0) {
      toast({ title: "Everything is already synced today" });
      return;
    }
    ...
    supabase.functions.invoke("prospects-daily-sync", {
      body: { priorityIds: filteredSorted.slice(0, 500).map(r => r.id), force: false },
    }).catch(() => {});
    ...
    while (!cancelSyncRef.current) {
      ...
      // Safety stop after 30 min so the spinner can't get stuck forever
      if (Date.now() - startedAt > 30 * 60 * 1000) break;
    }

    setSyncing(false);
    toast({ title: cancelSyncRef.current ? "Sync stopped" : `Synced ${initialPending} rows` });
  };
    ...
    <div className="h-screen flex flex-col bg-background text-foreground overflow-hidden">
        ...
        <div className="px-4 py-3 flex items-center gap-3 flex-wrap">
          ...
          {selected.size > 0 && (
            <div className="inline-flex items-center gap-1 rounded-md border border-border bg-background px-1.5 py-0.5">
              ...
              <Button size="icon" variant="ghost" className="h-7 w-7" onClick={openBulkAI} title="Bulk AI email">
                <Wand2 className="h-3.5 w-3.5 text-primary" />
              </Button>
              {isAdmin && sourceFilter !== "banned" && (
                <Button size="sm" variant="ghost" className="h-7 px-2 text-[11px] text-destructive hover:text-destructive" onClick={() => banSelected()} title="Move selected to Banned Leads">
                  Ban
                </Button>
              )}
              {isAdmin && sourceFilter === "banned" && (
          ...
          {/* Auth chip */}
          {role && (
            <div className="inline-flex items-center gap-2 px-2 h-9 rounded-md border border-border text-[11px] text-muted-foreground">
          ...
          <Button
            ...
            onClick={syncAll}
            title={syncing ? "Click to cancel sync" : "Sync all YT channel data now (auto runs daily at 03:30 UTC)"}
          >
            {syncing
              ? <Loader2 className="h-3.5 w-3.5 animate-spin text-primary" />
              : <RefreshCw className="h-3.5 w-3.5" />}
            <span className="text-xs">
              {syncing ? `Sync ${syncProgress.done}/${syncProgress.total}` : "Sync"}
            </span>
          </Button>
          ...
          <div className="inline-flex items-center rounded-md border border-border bg-background p-0.5" role="tablist">
            <button
              ...
              title="Rows on the prospect list"
            >
              Prospect List
            </button>
            <button
              ...
              <tr key={row.id} className={`hover:bg-muted/30 ${row.autoDiscovered ? "bg-amber-50/60 dark:bg-amber-950/20 border-l-2 border-amber-400" : ""}`} style={{ height: 34 }} title={row.autoDiscovered ? "Auto-discovered from Google Workspace conversations" : undefined}>
                ...
                {visibleColumns.map(col => {
                  if (col.type === "emails") {
                    ...
                    const hasReply = !!convReplied[row.id];
                    return (
                        ...
                        <div className="flex items-center justify-center gap-1.5">
                          <span
                            className={`inline-block h-2 w-2 rounded-full shrink-0 ${
                              cc === 0 ? "bg-muted-foreground/30"
                                : hasReply ? "bg-emerald-500 shadow-[0_0_6px_rgba(16,185,129,0.7)]"
                                : "bg-muted-foreground/50"
                            }`}
                            title={cc === 0 ? "No conversation yet" : hasReply ? "Prospect has replied" : "Sent — awaiting reply"}
                          />
                          <ProspectEmails
      ...
      <Sheet open={bulkAIOpen} onOpenChange={(o) => { if (!bulkSending) setBulkAIOpen(o); }}>
        ...
        >
          <SheetHeader className="px-6 pt-5 pb-3 border-b border-border shrink-0">
            <SheetTitle className="flex items-center gap-2 text-base">
              <Wand2 className="h-3.5 w-3.5 text-primary" /> Bulk AI Outreach · {bulkDrafts.length} lead{bulkDrafts.length === 1 ? "" : "s"}
            </SheetTitle>
          </SheetHeader>
          <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-3">
            ...
            {bulkGapMode === "random" ? (
              <div className="inline-flex items-center gap-1 text-[11px]">
                <Input
                  ...
                  onChange={e => setBulkAIMinSec(Math.max(30, parseInt(e.target.value.replace(/\D/g, "")) || 30))}
                  disabled={bulkSending}
                />
                ...
                <Input
                  ...
                  onChange={e => setBulkAIMaxSec(Math.max(30, parseInt(e.target.value.replace(/\D/g, "")) || 30))}
                  disabled={bulkSending}
                />
              ...
              <div className="inline-flex items-center gap-1 text-[11px]">
                <Input
                  ...
                  onChange={e => setBulkGapFixedSec(Math.max(30, parseInt(e.target.value.replace(/\D/g, "")) || 30))}
                  disabled={bulkSending}
                />
          ...
          {/* Bulk row actions inside the dialog: select all + bulk sender edit */}
