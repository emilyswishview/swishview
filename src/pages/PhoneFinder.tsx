...
const LS_PENDING = 'phoneFinder.pendingLeads';
...
type Settings = {
  ...
  minSubs: string;
  maxSubs: string;
  country: string;      // comma separated ISO codes applied after enrichment
...
const PhoneFinder = () => {
  ...
  const [s, setS] = useState<Settings>(() => {
    try {
      const st = { ...DEFAULTS, ...JSON.parse(localStorage.getItem(LS_KEY) || '{}') } as Settings;
      // Leads are US/CA only — repair older saved settings that had no filter.
      if (!st.country.trim()) st.country = 'US,CA';
      return st;
  ...
  const [phase, setPhase] = useState<'idle' | 'searching' | 'scanning' | 'waiting' | 'stopped' | 'done'>('idle');
  ...
  useEffect(() => {
    ...
    try { pendingRef.current = JSON.parse(localStorage.getItem(LS_PENDING) || '[]'); } catch {}
    setPendingCount(pendingRef.current.length);
    // Any lead that failed to save in a previous session is retried on load,
    // so a found phone number is never lost.
    if (pendingRef.current.length) void flushPending();
  }, []);
  const persistSeen = () => {
    try { localStorage.setItem(LS_SEEN, JSON.stringify(Array.from(seenRef.current).slice(-20000))); } catch {}
  };
  const persistPending = () => {
    try { localStorage.setItem(LS_PENDING, JSON.stringify(pendingRef.current.slice(-5000))); } catch {}
    setPendingCount(pendingRef.current.length);
  };
  ...
  // Writes a lead into calling_leads. Never throws away a lead: falls back to
  // the server-side upsert, then to a persisted retry queue.
  const persistLead = async (payload: any): Promise<'saved' | 'duplicate' | 'queued'> => {
    ...
    const { data: rpcId, error: rpcErr } = await supabase.rpc('upsert_calling_lead_from_channel', {
      _channel_id: payload.channel_id || '',
      _channel_name: payload.channel_name || '',
      _channel_link: payload.channel_link || '',
      _thumbnail: payload.thumbnail || '',
      _phone: payload.phone || '',
      _subscribers: payload.subscribers || 0,
      _total_views: payload.total_views || 0,
      _country: payload.country || '',
      _keyword: payload.keyword || '',
    });
    if (!rpcErr && rpcId) return 'saved';

    if (!pendingRef.current.some((p) => p.channel_id === payload.channel_id)) {
      pendingRef.current.push(payload);
      persistPending();
    }
    addLog(`⚠ ${payload.channel_name}: save failed (${rpcErr?.message || error.message}) — kept in retry queue`);
    return 'queued';
  };
  ...
  const flushPending = async () => {
    ...
    for (const payload of queue) {
      const { error } = await supabase.from('calling_leads').insert(payload);
      if (!error || String(error.code) === '23505' || /duplicate key/i.test(error.message)) { recovered++; continue; }
      const { data: rpcId, error: rpcErr } = await supabase.rpc('upsert_calling_lead_from_channel', {
        _channel_id: payload.channel_id || '',
        _channel_name: payload.channel_name || '',
        _channel_link: payload.channel_link || '',
      ...
      if (!rpcErr && rpcId) { recovered++; continue; }
      stillFailing.push(payload);
    }
    pendingRef.current = stillFailing;
    persistPending();
    if (recovered) addLog(`↻ Recovered ${recovered} queued lead(s) into the Calling tab.`);
    if (stillFailing.length) addLog(`⚠ ${stillFailing.length} lead(s) still unsaved — they stay queued and will retry.`);
  };
  ...
  const saveLead = async (row: Row): Promise<'saved' | 'duplicate' | 'error'> => {
    ...
    const res = await persistLead(payload);
    if (res === 'queued') return 'error';
    return res;
  };


  const scrapeBatch = async (batch: Row[], concurrency: number, allowed: string[] = []) => {
    let idx = 0;
    await Promise.all(
      Array.from({ length: Math.min(concurrency, batch.length) }, async () => {
        while (idx < batch.length && !cancelRef.current) {
          ...
          try {
            ...
            const country = (enriched.country || '').toUpperCase();
            if (allowed.length && country && !allowed.includes(country)) {
              update(row.channelId, { ...enriched, status: 'none' });
              addLog(`↷ ${row.title} skipped — country ${country} not in ${allowed.join('/')}`);
              continue;
            }
            if (phone) {
              ...
              update(row.channelId, { ...enriched, status: 'none' });
              addLog(`· ${row.title} — no public phone found`);
            }
          } catch (e: any) {
            setScannedTotal(n => n + 1);
            // Un-mark it as seen so a transient scrape failure never permanently
            // drops the channel — the next round can pick it up again.
            seenRef.current.delete(row.channelId);
            persistSeen();
            update(row.channelId, { status: 'error', error: e?.message || 'failed' });
            addLog(`✕ ${row.title} — scrape error: ${e?.message || 'failed'} (will be retried later)`);
          }
  ...
  const run = async () => {
    ...
    if (!Object.keys(tokensRef.current).length) {
      setRows([]);
      setLog([]);
      setSaved(0);
      setDupes(0);
    ...
    const allowed = s.country.split(',').map(c => c.trim().toUpperCase()).filter(Boolean);
    let pageTokens: Record<string, string> = { ...tokensRef.current };
    let processed = 0;
    let round = 0;

    const resuming = Object.keys(pageTokens).length > 0;
    addLog(`${resuming ? 'Resuming' : 'Starting'}: ${keywords.length} keyword(s) · cap ${cap} · countries ${allowed.join('/') || 'any'} · min subs ${minSubs || 'any'}${s.continuous ? ' · continuous ON' : ''}`);
    ...
    while (!cancelRef.current && processed < cap) {
      ...
      let dropSubs = 0, dropCountry = 0;
      const filtered = fresh.filter(r => {
        const subs = r.subscribers ?? 0;
        if ((minSubs && subs < minSubs) || (maxSubs && subs > maxSubs)) { dropSubs++; return false; }
        const c = (r.country || '').toUpperCase();
        // Unknown country is kept here — it is re-checked after the channel
        // scrape returns the real country, so we never lose valid US/CA leads.
        if (allowed.length && c && !allowed.includes(c)) { dropCountry++; return false; }
        return true;
      });

      const batch = filtered.slice(0, Math.max(0, cap - processed));
      addLog(`round ${round}: ${all.length} returned · ${fresh.length} new · ${filtered.length} pass filters (${dropSubs} by subs, ${dropCountry} outside ${allowed.join('/') || 'any'}) → scraping ${batch.length}`);

      if (batch.length) {
        setRows(rs => [...batch, ...rs].slice(0, 1500));
        setPhase('scanning');
        await scrapeBatch(batch, concurrency, allowed);
    ...
    const pending = Object.keys(tokensRef.current).length;
    if (stopped) {
      addLog(`⏸ Stopped by you after ${processed} channel(s). ${pending ? `${pending} keyword(s) have a saved position — press “Resume scraping” to continue exactly where it left off.` : 'Press “Find & add to Calling” to start a fresh walk.'}`);
      setResumeInfo(pending
        ? `Paused. ${pending} keyword position(s) saved — resume continues from there.`
        : 'Paused. No saved position, the next run starts a fresh keyword walk.');
    } else {
      addLog(`✔ Finished. Scraped ${processed} channels this run.`);
      setResumeInfo('');
    }
    setPhase(stopped ? 'stopped' : 'done');
    setBusy(false);
    toast({
      title: stopped ? 'Scraping paused' : 'Phone scan finished',
      description: stopped
        ? `${processed} channels scanned. Resume anytime.`
        : `${processed} channels scanned.`,
    });
  };
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <Card className="p-4 space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-4">
            ...
            <div>
              <label className="text-[11px] uppercase tracking-wide text-muted-foreground">Countries (comma ISO)</label>
              <Input value={s.country} placeholder="US,CA" disabled={busy}
                onChange={(e) => set('country', e.target.value.toUpperCase().replace(/[^A-Z, ]/g, ''))} />
            </div>
            <div>
          ...
          <div className="flex flex-wrap gap-3 items-center pt-1">
            <Button onClick={run} disabled={busy || !s.query.trim()}>
              ...
              {busy ? 'Working…' : (phase === 'stopped' && Object.keys(tokensRef.current).length ? 'Resume scraping' : 'Find & add to Calling')}
            </Button>
            {busy && (
              <Button variant="outline" onClick={() => { cancelRef.current = true; setPhase('waiting'); addLog('Stop requested — finishing the channels already in flight…'); }}>
            ...
            <Button
              variant="ghost"
              disabled={busy}
              onClick={() => { seenRef.current = new Set(); tokensRef.current = {}; persistSeen(); setResumeInfo(''); addLog('Cleared local seen-channel memory and saved keyword positions.'); }}
          ...
          {resumeInfo && (
            <p className="text-xs font-medium text-amber-600">{resumeInfo}</p>
          )}
