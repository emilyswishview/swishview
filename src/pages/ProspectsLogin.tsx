...
const ProspectsLogin = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data.session?.user && role) {
        // Validate the stored token against the server. A dead/revoked token
        // (left over after a password reset) still parses locally and would
        // bounce us to /prospects only to be logged straight back out.
        const { data: u, error } = await supabase.auth.getUser();
        if (!error && u.user && deriveProspectsRole(u.user.email)) {
          navigate("/prospects");
          return;
        }
        // Stale session — purge it so the form below starts clean.
      ...
      } else if (data.session?.user && !role) {
  ...
  // Wipe any stale/dead Supabase session from this browser so a fresh sign-in
  // starts clean. Old refresh tokens (e.g. left over after a server-side
  // password reset / session purge) otherwise cause an immediate auto-logout.
