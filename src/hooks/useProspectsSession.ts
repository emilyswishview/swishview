...
export const PROSPECTS_ALLOWED_EMAILS = [
  ...
  "marcus@swishview.com",
] as const;

// View-only accounts: read-only everywhere, with contact details (email /
// alt email / phone) hidden from the table.
export const PROSPECTS_VIEWER_EMAILS = ["marcus@swishview.com"] as const;

export function isProspectsViewer(email: string | null | undefined): boolean {
  if (!email) return false;
  return (PROSPECTS_VIEWER_EMAILS as readonly string[]).includes(email.trim().toLowerCase());
}
...
export const PROSPECTS_USER_DISPLAY: Record<string, string> = {
  ...
  "marcus@swishview.com": "Marcus (View only)",
};
...
export function useProspectsSession(): ProspectsSession {
  ...
  const isViewer = isProspectsViewer(email);
  return {
    loading, userId, email, role,
    // Viewers (Marcus) see the full admin UI — every tab and panel an admin
    // sees — but stay read-only (perms below) and have contacts masked.
    isAdmin: isAdmin || isViewer,
    isViewer,
    maskContacts: isViewer,
    perms: isViewer ? READ_ONLY_PERMS : (isAdmin ? ALL_PERMS : perms),
    signOut,
  };
}
