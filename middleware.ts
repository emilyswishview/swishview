...
export default async function middleware(req: Request) {
  ...
  try {
    ...
    // If prerender fails (bad token, quota, 5xx) or returns a non-HTML body,
    // fall back to the SPA so crawlers still get real HTML.
    const prerenderType = prerenderRes.headers.get('content-type') || '';
    if (!prerenderRes.ok || !prerenderType.includes('html')) {
      return next();
    }
