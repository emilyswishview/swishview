...
// Build the per-recipient unsubscribe URL. Lands on swishview.com home,
// which shows a small corner card; confirming there POSTs the token to the
// prospect-unsubscribe edge function and suppresses future sends.
const UNSUB_BASE = "https://www.swishview.com/?unsubscribe=";
const b64urlEncode = (s: string): string => {
  // deno-lint-ignore no-explicit-any
  const b: string = (globalThis as any).btoa(unescape(encodeURIComponent(s)));
  return b.replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "");
};
const unsubUrlFor = (email: string) => UNSUB_BASE + b64urlEncode(email.trim().toLowerCase());
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // --- Suppression check: skip anyone who unsubscribed -------------------
    try {
      const supabaseUrl = Deno.env.get("SUPABASE_URL");
      const serviceKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
      if (supabaseUrl && serviceKey) {
        const supa = createClient(supabaseUrl, serviceKey, { auth: { persistSession: false } });
        const lowered = cleanTo.map(e => e.toLowerCase());
        const { data: hits } = await supa
          .from("prospects")
          .select("data")
          .in("data->>email", lowered as any)
          .eq("data->>unsubscribed", "true" as any)
          .limit(1);
        if (hits && hits.length) {
          return new Response(
            JSON.stringify({ ok: false, error: "Recipient previously unsubscribed", suppressed: true }),
            { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } },
          );
        }
      }
    } catch (e) {
      console.warn("suppression check failed (continuing):", (e as any)?.message || e);
    }
    ...
    // Per-recipient unsubscribe link. If multiple recipients, link uses the first.
    const unsubUrl = unsubUrlFor(cleanTo[0] || "");
    const footerText = `\n\n—\nIf you'd rather not hear from us, you can unsubscribe here: ${unsubUrl}`;
    const footerHtml =
      `<p style="margin:28px 0 0 0;padding-top:14px;border-top:1px solid #eaeaea;font-size:12px;line-height:1.5;color:#888888;">`
      + `Don't want emails from us? `
      + `<a href="${unsubUrl}" style="color:#888888;text-decoration:underline;">Unsubscribe</a>.`
      + `</p>`;
    text = text + footerText;
    let htmlToSend = safeHtml.trim() && !body.text?.trim()
      ? safeHtml
      : textToHtml(text.replace(footerText, "")); // textToHtml re-adds footerHtml itself
    // If we're sending raw safeHtml (user-supplied), append the footer once.
    if (safeHtml.trim() && !body.text?.trim() && !htmlToSend.includes(unsubUrl)) {
      htmlToSend = htmlToSend.replace(/<\/body>\s*<\/html>\s*$/i, `${footerHtml}</body></html>`);
      if (!htmlToSend.includes(unsubUrl)) htmlToSend = htmlToSend + footerHtml;
    }
