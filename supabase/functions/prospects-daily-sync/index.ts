...
const CONCURRENCY = 16;         // parallel YT lookups per chunk
...
async function processOne(supabase: any, row: any, force = false): Promise<boolean> {
  ...
  try {
    ...
    if (error || !yt || yt.error) {
      // Mark the attempt so we don't keep retrying the same broken link in tight loops.
      await supabase.from("prospects")
        .update({ data: { ...data, lastFetchedAt: new Date().toISOString(), lastSyncError: (error?.message || yt?.error || "unknown") } })
        .eq("id", row.id);
      return false;
    }
...
async function selfInvoke(force: boolean) {
  // Fire-and-forget HTTP to self so a new invocation picks up the remaining rows.
  try {
    await fetch(`${SUPABASE_URL}/functions/v1/prospects-daily-sync`, {
      ...
      body: JSON.stringify({ continuation: true, force }),
    });
  } catch (e) {
...
async function runSync(priorityIds: string[] = [], force = false) {
  ...
  // 1) Priority IDs first (the rows currently visible in the user's filter)
  if (priorityIds.length) {
    const { data: prio } = await supabase
      .from("prospects")
      .select("id,data,channel_link,last_fetched_at,channel_name")
      .in("id", priorityIds);
    if (prio?.length) {
  ...
  while (true) {
    ...
    let q = supabase
      ...
      .limit(FETCH_PAGE);
    if (!force) {
      // Skip rows already synced today. Using ISO date boundary.
      // BUT include rows that are missing channel_name (sync produced no data).
      q = q.or(
        `last_fetched_at.is.null,last_fetched_at.lt.${today}T00:00:00Z,channel_name.is.null,channel_name.eq.`
      );
    }
    ...
    // If the batch was smaller than FETCH_PAGE we've reached the tail.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    (globalThis as any).EdgeRuntime?.waitUntil(runSync(priorityIds, force));

    return new Response(JSON.stringify({ ok: true, started: true, priorityIds: priorityIds.length }), {
