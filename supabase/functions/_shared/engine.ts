...
function classify(status: number, body: any): "daily" | "rate" | "key" | "other" {
  ...
  if (status === 403 && /quota/i.test(msg)) return "rate";
...
/** Next YouTube daily-quota reset — midnight Pacific time, as a UTC ISO string. */
export function quotaResetAt(from = new Date()): string {
  const ptNaive = new Date(from.toLocaleString("en-US", { timeZone: "America/Los_Angeles" }));
  const offset = from.getTime() - ptNaive.getTime();
  const nextMidnight = new Date(ptNaive);
  nextMidnight.setHours(24, 0, 0, 0);
  return new Date(nextMidnight.getTime() + offset).toISOString();
}

/**
 * Zero the per-project counters when the Pacific day has changed.
 * Must run before ANY read of the counters, otherwise a fully-used pool looks
 * exhausted forever and the engine never calls reserve_api_quota (which used to
 * be the only place the daily rollover happened).
 */
export async function rollQuotaDay(sb: Sb): Promise<void> {
  const { error } = await sb.rpc("roll_api_quota_day");
  if (!error) return;
  // Fallback when the helper RPC has not been installed yet: do the rollover inline
  // so an old Pacific day can never freeze the whole pool as "exhausted".
  try {
    const today = new Date(new Date().toLocaleString("en-US", { timeZone: "America/Los_Angeles" }))
      .toISOString().slice(0, 10);
    await sb.from("youtube_api_projects")
      .update({
        search_calls_used: 0, read_units_used: 0, quota_day: today,
        health_status: "healthy", cooldown_until: null, error_count: 0,
      })
      .lt("quota_day", today);
    // also clear cooldowns that have already elapsed
    await sb.from("youtube_api_projects")
...
/** Live picture of the API pool: how many projects can still serve a search call. */
export async function quotaStatus(sb: Sb): Promise<{
  available: number; keyed: number; total: number; exhausted: boolean; resetAt: string;
}> {
  await rollQuotaDay(sb);
  const { data } = await sb.from("youtube_api_projects")
    .select("enabled,health_status,search_calls_used,search_calls_limit,read_units_used,read_units_limit,api_key_secret_name,cooldown_until")
    .eq("enabled", true);
  const rows = data || [];
  const keyed = rows.filter((p: any) => !!apiKeyFor(p.api_key_secret_name));
  // A project can only serve another search when it has both a search call and
  // 100 read units left — read units are what actually run out first.
  const available = keyed.filter((p: any) =>
    p.health_status !== "exhausted"
    && p.search_calls_used < p.search_calls_limit
    && Number(p.read_units_used || 0) + 100 <= Number(p.read_units_limit || 0)).length;
  return {
    available,
    keyed: keyed.length,
    total: rows.length,
...
export async function allProjectsExhausted(sb: Sb): Promise<boolean> {
  await rollQuotaDay(sb);
  const { data } = await sb.from("youtube_api_projects").select("enabled,health_status,search_calls_used,search_calls_limit,read_units_used,read_units_limit").eq("enabled", true);
  if (!data?.length) return true;
  return data.every((p: any) => p.health_status === "exhausted"
    || p.search_calls_used >= p.search_calls_limit
    || Number((p as any).read_units_used || 0) + 100 > Number((p as any).read_units_limit || 0));
}
