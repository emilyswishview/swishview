...
import {
  svc, workerId, getSettings, pauseEngine, ytApi, allProjectsExhausted, quotaStatus,
...
// Discovery targets podcasters and musicians — the niches we actually sell to.
const NICHE_SEED = [
  "podcast", "podcast channel", "video podcast", "interview podcast",
  "true crime podcast", "comedy podcast", "business podcast", "health podcast",
  "sports podcast", "tech podcast", "independent musician", "singer songwriter",
  "indie band", "hip hop artist", "rapper", "music producer",
  "cover songs artist", "original music artist", "guitarist", "pianist",
  "electronic music producer", "country music artist", "rock band", "r&b singer",
];
...
async function sweepQueue(ctx: Ctx) {
  ...
  // a 'cooling'/'error' row with no cooldown timestamp would stay stuck forever
  await sb.from("youtube_api_projects")
    .update({ health_status: "healthy", error_count: 0 })
    .in("health_status", ["cooling", "error"]).is("cooldown_until", null);

}

// ───────────────────────────────────────────────── frontier

/** The only niches the engine may search right now. */
function activeNiches(cfg: any): string[] {
  return cfg?.niches?.length ? cfg.niches : NICHE_SEED;
}

/**
 * Retire every segment whose niche is no longer targeted and drop any pending
 * search job pointing at one, so old niches (fitness, real estate, …) can never
 * be executed again after the niche list changes.
 */
async function purgeOffNicheFrontier(ctx: Ctx) {
  const { sb } = ctx;
  const niches = activeNiches(ctx.cfg);
  const list = `(${niches.map((n) => `"${String(n).replace(/"/g, '')}"`).join(",")})`;

  const { data: stale } = await sb.from("search_segments")
    .select("id")
    .not("niche", "in", list)
  ...
  await sb.from("search_segments").update({ status: "retired", priority: 0 }).in("id", ids);

  // remove queued/claimed jobs that reference the retired segments
  const { data: pending } = await sb.from("discovery_jobs")
    .select("id,payload").eq("job_type", "search")
    .in("status", ["queued", "retry", "claimed", "running"]).limit(5000);
  const kill = (pending || []).filter((j: any) => ids.includes(j.payload?.segment_id)).map((j: any) => j.id);
  if (kill.length) await sb.from("discovery_jobs").delete().in("id", kill);

  ctx.log.push(`purged off-niche frontier: ${ids.length} segment(s) retired, ${kill.length} stale search job(s) removed`);
}

async function refillFrontier(ctx: Ctx, force = false) {
  const { sb, cfg } = ctx;
  await purgeOffNicheFrontier(ctx);
  ...
  const niches: string[] = activeNiches(cfg);
  ...
  // Count only segments for the CURRENT niche list — otherwise a niche change
  // (e.g. switching to podcasts/musicians) never seeds because stale segments
  // from the old niches already push the total above `expected`.
  ...
  let segs = await openSegments();

  // Frontier rotation: when every segment in the target markets is retired the engine
  // would idle forever. Revive the least-recently-searched retired segments from page 1
  // so discovery keeps running (channels change over time, and dedupe protects us).
  if (!segs.length) {
    const rescanCutoff = new Date(Date.now() - 5 * 24 * 3600_000).toISOString();
    let revive = sb.from("search_segments").select("id")
      .in("status", ["exhausted", "low_yield"])
      .in("niche", niches);
    if (regions.length) revive = revive.in("region_code", regions);
    revive = revive.or(`last_searched_at.is.null,last_searched_at.lt.${rescanCutoff}`);
    const { data: dead } = await revive
      .order("last_searched_at", { ascending: true, nullsFirst: true }).limit(40);
    const ids = (dead || []).map((r: any) => r.id);
    if (ids.length) {
      await sb.from("search_segments")
        .update({ status: "new", page_token: null, priority: 40 })
        .in("id", ids);
      ctx.log.push(`frontier rotation: revived ${ids.length} retired segment(s) in ${regions.join('/') || 'all regions'}`);
      segs = await openSegments();
    } else {
      ctx.log.push(`frontier empty: no segments left for markets ${regions.join('/') || '(none configured)'} — add niches or markets in the engine config`);
    }
  }
...
async function runSearchJob(ctx: Ctx, job: any): Promise<{ exhausted?: boolean }> {
  ...
  // Last line of defence: never spend a quota unit on a niche we no longer target.
  if (!activeNiches(cfg).includes(seg.niche)) {
    await sb.from("search_segments").update({ status: "retired", priority: 0 }).eq("id", seg.id);
    await finishJob(sb, job.id, true);
    ctx.log.push(`skipped off-niche segment "${seg.niche}" (${seg.query})`);
    return {};
  }


  const fingerprint = pageFingerprint(seg.search_signature, seg.page_token);
  const insertSearchJob = () => sb.from("search_jobs").insert({
  ...
  let { data: sj, error: sjErr } = await insertSearchJob();

  // RESCAN_AFTER_MS: a page searched long ago is worth searching again — YouTube keeps
  // publishing new channels for the same query, and dedupe still blocks known channels.
  const RESCAN_AFTER_MS = 5 * 24 * 3600_000;
  if (sjErr && /duplicate|23505/i.test(`${sjErr.code} ${sjErr.message}`)) {
    const { data: prev } = await sb.from("search_jobs")
      .select("id,created_at").eq("page_fingerprint", fingerprint)
      .order("created_at", { ascending: false }).limit(1).maybeSingle();
    const age = prev?.created_at ? Date.now() - new Date(prev.created_at).getTime() : Infinity;
    if (prev?.id && age > RESCAN_AFTER_MS) {
      await sb.from("search_jobs").delete().eq("page_fingerprint", fingerprint);
      const retry = await insertSearchJob();
      sj = retry.data; sjErr = retry.error;
      if (!sjErr) ctx.log.push(`re-scanning stale page ${fingerprint} (last searched ${Math.round(age / 86_400_000)}d ago)`);
    }
  }

  if (sjErr) {
    // This exact page ran recently — never repeat a fresh search. Retire the segment so
    // the planner stops re-selecting it every tick (pure duplicate work).
...
serve(async (req) => {
  ...
  try {
    ...
    // Daily-quota gate. YouTube quota resets at midnight Pacific time; until then no
    // search.list call can succeed, so say so plainly instead of looking "busy".
    const quota = await quotaStatus(sb);
    const waitMs = new Date(quota.resetAt).getTime() - Date.now();
    if (quota.exhausted) {
      const h = Math.floor(waitMs / 3_600_000);
      const m = Math.max(0, Math.round((waitMs % 3_600_000) / 60_000));
      log.push(
        `daily YouTube quota exhausted — 0 of ${quota.keyed} keyed project(s) available. ` +
        `Search resumes automatically at the Pacific-midnight reset in ${h}h ${m}m. Contact scraping still runs.`,
      );
    }
    ...
    while (Date.now() - startedAt < BUDGET_MS && !exhausted) {
      ...
      const { data: searchJobs } = quota.exhausted ? { data: [] as any[] } : await sb.rpc("claim_jobs", {
    ...
    // Out of quota is NOT a failure: autopilot stays on and simply waits for the
    // Pacific-midnight reset, so nothing has to be re-enabled by hand.
    if (exhausted && await allProjectsExhausted(sb)) {
      log.push("every API project is out of daily quota — waiting for the Pacific-midnight reset (autopilot stays on)");
    }
    ...
    try {
      ...
      // Phone repair is a full-table sweep: run it a couple of times an hour, not
      // on every minute tick — hourly ticks were saturating the database.
