...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Revoke all existing sessions for this user so anyone signed in with the
    // old password is forced to sign in again.
    try {
      // @ts-ignore - second arg "global" supported by gotrue admin API
      const { error: soErr } = await admin.auth.admin.signOut(target.id, "global");
      if (soErr) console.warn("[reset-pw] signOut warning", soErr.message);
    } catch (e) {
      console.warn("[reset-pw] signOut threw", (e as Error).message);
    }

    // Broadcast a realtime event so any device currently signed in as the
    // target user can sign itself out within milliseconds (instead of waiting
    // for the next refresh-token poll to fail).
    try {
      const resp = await fetch(`${SUPABASE_URL}/realtime/v1/api/broadcast`, {
        ...
        body: JSON.stringify({
          messages: [{
            topic: "prospects-auth",
            event: "password-reset",
            payload: { email, userId: target.id, at: Date.now() },
            private: false,
          }],
        }),
      });
      if (!resp.ok) console.warn("[reset-pw] broadcast failed", resp.status, await resp.text());
    } catch (e) {
      console.warn("[reset-pw] broadcast threw", (e as Error).message);
    }
