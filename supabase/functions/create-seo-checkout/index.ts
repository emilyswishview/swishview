...
serve(async (req) => {
  ...
  try {
    ...
    // Derive Stripe recurring interval from plan duration.
    // Stripe allows interval_count up to 12 for month intervals — SEO plans are 3/6/9/12 months.
    const durationMonths = Number(seoPlan.duration_months) || 1;
    const intervalCount = Math.min(Math.max(durationMonths, 1), 12);
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: `YouTube SEO Optimization — ${cycleLabel} billing (${durationMonths}-month package)`,
            },
